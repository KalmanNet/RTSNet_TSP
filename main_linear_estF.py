...
for index in range(0,len(r2)):
   ...
   dataFileName = ['2x2_Frot10_rq-1010_T100.pt','2x2_Frot10_rq020_T100.pt','2x2_Frot10_rq1030_T100.pt','2x2_Frot10_rq2040_T100.pt','2x2_Frot10_rq3050_T100.pt']
   ...
   [MSE_KF_linear_arr_partialh, MSE_KF_linear_avg_partialh, MSE_KF_dB_avg_partialh] = KFTest(sys_model_partialf, test_input, test_target)
   ...
   [MSE_RTS_linear_arr_partialh, MSE_RTS_linear_avg_partialh, MSE_RTS_dB_avg_partialh] = S_Test(sys_model_partialf, test_input, test_target)
   ...
   X = torch.squeeze(train_target[:,:,0]).to(dev,non_blocking = True)
   Y = torch.squeeze(train_target[:,:,1]).to(dev,non_blocking = True)
   ...
   Y_1 = torch.unsqueeze(Y[:,0],1)
   Y_2 = torch.unsqueeze(Y[:,1],1)
   F_row1 = torch.matmul(torch.matmul(torch.inverse(torch.matmul(X.T,X)),X.T),Y_1).to(dev,non_blocking = True)
   F_row2 = torch.matmul(torch.matmul(torch.inverse(torch.matmul(X.T,X)),X.T),Y_2).to(dev,non_blocking = True)
