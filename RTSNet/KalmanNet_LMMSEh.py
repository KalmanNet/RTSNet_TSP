...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitSequence(self, M1_0, T):
        ...
        self.y_previous = torch.squeeze(self.m1x_posterior).to(dev, non_blocking=True)
    ...
    ### Compute Priors ###
    ######################
    def step_prior(self, y):
        # Predict the 1-st moment of x
        self.m1x_prior = torch.squeeze(self.f(self.m1x_posterior))

        # Compute the Jacobians
        self.UpdateJacobians(getJacobian(self.m1x_prior, self.h))

        # Predict the 1-st moment of y
        self.m1y = torch.squeeze(self.m1x_prior)# H=I
        H_lmmse = torch.matmul(torch.inverse(torch.matmul(self.H_T,self.H)),self.H_T)
        self.lmmse_y = torch.squeeze(torch.matmul(H_lmmse, y))
    
    def UpdateJacobians(self, H):
        self.H = H
    ...
    def KNet_step(self, y):
        ...
        # Compute Kalman Gain
        self.step_KGain_est(self.lmmse_y)
