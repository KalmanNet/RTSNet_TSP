...
def PFTest(SysModel, test_input, test_target, n_part=100, init_cond=None):
    ...
    PF_out = torch.empty((N_T, test_target.size()[1], SysModel.T_test))
    ...
    for j in range(N_T):
        ...
        PF_out[j, :, :] = torch.from_numpy(sim.get_filtered_mean()[1:,].T) # start from T=1 since 0 is for x0


    for j in range(N_T):
        MSE_PF_linear_arr[j] = loss_fn(PF_out[j, :, :], test_target[j, :, :])
